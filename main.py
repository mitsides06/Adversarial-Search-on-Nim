...
class Game:
    ...
    def max_decision(self, state):
        """ Returns the Minimax decision(s) of the Max (user) player.

            Args:
                state (list) : current state of the game
            
            Returns:
                list[tuple] : list of possible next optimal actions presented as tuples.
        """
        alpha = -float("inf")
        beta = float("inf")
...
if __name__ == "__main__":
    ...
    times, states_visited = nim.play(m, n, k, is_pruned=True)
